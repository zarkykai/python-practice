...
import Reader       # 文件读取方法存储在自定义的Reader模块中
...
#################
    ...
    for counter in range(len(list_copy)):
        ...
        '''判断输入的步长的正负-----------------------'''
        if step > 0:
            start_point = (start_point - 1 + step) % rem

        if step < 0:
            start_point = (start_point + 1 + step) % rem
        '''-----------------------------------------------'''

        death_list.append(list_copy[start_point])
        del list_copy[start_point]
...
class JosephusRing:
    ...
    def __init__(self, ipt_list, step=0, start_person=''):
        ...
        self.death_list = []
        self.__pointer = self.set_pointer()
    ...
    def __next__(self):
        if len(self.__list) > 0:
            ...
            self.death_list.append(self.__list[self.__pointer])     # 把索引指定值加入death_list中
            del self.__list[self.__pointer]

        else:
            raise StopIteration
        return self.death_list[-1]   # 返回死亡名单中的最新一个对象
